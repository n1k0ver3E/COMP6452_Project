...
contract('ProductSC', (accounts) => {
    ...
    it('createProduct() successful Test)', async () => {
        const productId = await product.createProduct.call("Potato",123,345,"syd", { from: farmer });
        await product.createProduct("Potato",123,345,"syd", { from: farmer });

        assert.equal(productId, 1, "ProductID should be equal to 1");
        const result = await product.products.call(1);
        assert.equal(result.productName, "Potato", 'ProductName should be potato');
    });
    ...
    it('purchasingProductInfo() successful Test)', async () => {
        let purchase = await product.purchasingProductInfo.call(1,30, { from: consumer });
        await product.purchasingProductInfo(1,30, { from: consumer });

        const result = await product.products.call(1);
        // assert.equal(result.statusType.toNumber(), 4, 'Status should be 4 in purchasing process');
        assert.equal(purchase_process.price, 30, "price should be in mapping collection.");
    });
    ...
    it('recallProduct() successful Test for reasonable role.)', async () => {
        let recall = await product.recallProduct.call(1, { from: farmer });
        await product.recallProduct(1,  { from: farmer });

        truffleAssert.eventEmitted(recall, 'RecallProduct', (ev) => {
            return ev.productId == 1;
        });
        
        const result = await product.products.call(1, { from: farmer });
