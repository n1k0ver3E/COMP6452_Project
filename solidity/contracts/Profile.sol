...
contract Profile {
    ...
    enum AccountType {Regulator,Farmer,Manufacturer,Retailer,Consumer,Logistic,Oracle}
    ...
    enum RegisState {Enable,Disable}
    enum MachineState {Activated, Deactivated}
    ...
    constructor (address _regulatorAddress, string memory _regulatorName) {
        ...
        Account memory regulatorAcc = Account(_regulatorAddress,_regulatorName,lastAccountId,AccountType.Regulator,AccountStatus.Approved,true,block.number);
   ...
   /// @param _accountTypeValue Specific the interger as following  0=Regulator,1=Farmer,2=Manufacturer,3=Reatiler,4=Logistic
    function registerAccount(address _accountAddress, string memory _accountName, uint256 _accountTypeValue) public checkDuplicateAddress(_accountAddress) onlyActiveRegisState onlyActivatedMachineState{
        require(_accountTypeValue != 0, 'Cannot register as the regulator.');
        require(msg.sender == _accountAddress,"The account address must be the same as the sender's address");
        uint256 tmpNewId = lastAccountId+1;
        Account memory newAcc = Account(_accountAddress,_accountName,tmpNewId,AccountType(_accountTypeValue),AccountStatus.Pending,true,block.number);
   ...
   /// @param _accountStatus 0=Pending, 1=Approved, 2=Rejected
    function approveAccount(address _accountAddress,uint256 _accountStatus) public onlyRegulator onlyActivatedMachineState{
   ...
   /// @param _accountAddress Each participant can have only one accountAddress for this phase. 
    function getAccountInfoByAddress(address _accountAddress) public view onlyActivatedMachineState returns(string memory accountName,uint256  accountId,uint256 accountTypeValue,uint256 accountStatusValue){
    ...
    function isAccountOwner(address _accountAddress, uint256 _accountId) public view onlyActivatedMachineState returns(bool){
        Account memory acc = accountInfoByAddress[_accountAddress];
    ...
    function isLogisticOrOracle(address _accountAddress) public view onlyActivatedMachineState returns(bool){
        if (bytes(accountInfoByAddress[_accountAddress].accountName).length != 0){ 
            if((accountInfoByAddress[_accountAddress].accountStatus == AccountStatus.Approved )&& 
            (accountInfoByAddress[_accountAddress].accountType == AccountType.Oracle ||
            accountInfoByAddress[_accountAddress].accountType == AccountType.Logistic))
            {
                return true;
            }
            else
    ...
    function isFarmer(address _accountAddress) public view onlyActivatedMachineState returns(bool){
        if (bytes(accountInfoByAddress[_accountAddress].accountName).length != 0){ 
            if((accountInfoByAddress[_accountAddress].accountType == AccountType.Farmer)&& (accountInfoByAddress[_accountAddress].accountStatus == AccountStatus.Approved ))
            {
                return true;
            }
            else
    ...
    function isManufacturer(address _accountAddress) public view onlyActivatedMachineState returns(bool){
        if (bytes(accountInfoByAddress[_accountAddress].accountName).length != 0){ 
            if((accountInfoByAddress[_accountAddress].accountType == AccountType.Manufacturer )
            && (accountInfoByAddress[_accountAddress].accountStatus == AccountStatus.Approved ))
            {
                return true;
            }
            else
    ...
    function isRetailer(address _accountAddress) public view onlyActivatedMachineState returns(bool){
        if (bytes(accountInfoByAddress[_accountAddress].accountName).length != 0){ 
            if((accountInfoByAddress[_accountAddress].accountType == AccountType.Retailer )
            && (accountInfoByAddress[_accountAddress].accountStatus == AccountStatus.Approved ))
            {
                return true;
            }
    ...
    function isConsumer(address _accountAddress) public view onlyActivatedMachineState returns(bool){
        if (bytes(accountInfoByAddress[_accountAddress].accountName).length != 0){ 
            if((accountInfoByAddress[_accountAddress].accountType == AccountType.Consumer )
            && (accountInfoByAddress[_accountAddress].accountStatus == AccountStatus.Approved ))
            {
                return true;
            }
    ...
    function getRegulatorAddress() public view onlyActivatedMachineState returns(address){
        address _regulatorAddress = regulatorAddress;
        return _regulatorAddress;
    }
    
        // Does account address exist?
    function isExisAccount(address _accountAddress) private view onlyActivatedMachineState returns(bool){
    ...
    function setEnableRegisState() public onlyRegulator onlyActivatedMachineState returns (uint256){
        // Enable registration
    ...
    function setDisableRegisState() public onlyRegulator onlyActivatedMachineState returns (uint256){
    ...
    /// @notice Disable registration fn
    function setDeactivatedMachineState() public onlyRegulator returns (uint256){
    ...
    function destroyContract() public onlyRegulator payable {
        // Disable registration
        regisState = RegisState.Disable;
   ...
   modifier checkDuplicateAddress(address _accountAddress){
    ...
    modifier onlyActiveRegisState{
        require(regisState == RegisState.Enable, 'The registration function is not enable.');
       _;
    }
    modifier onlyActivatedMachineState{
        require(machineState == MachineState.Activated, 'This contract is not activated by the regulator.');
       _;
    }
}
